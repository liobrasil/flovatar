...
access(all) contract PublicPriceOracle {
    ...
    /// Events
    access(all) event OracleAdded(oracleAddr: Address)
    access(all) event OracleRemoved(oracleAddr: Address)
    ...
    access(all) view fun getLatestPrice(oracleAddr: Address): UFix64 {
        ...
        let priceReaderRef = PublicPriceOracle.account.storage.borrow<&{OracleInterface.PriceReader}>(from: priceReaderSuggestedPath)
    ...
    access(all) resource Admin {
        ...
        access(all) fun removeOracle(oracleAddr: Address) {
