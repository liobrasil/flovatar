...
access(all) contract FIND {
    ...
    */
    access(all) resource interface LeaseCollectionPublic {
        ...
        access(all) fun adminAddAddon(name:String, addon: String)
        access(all) fun getAddon(name:String) : [String]
        access(all) fun checkAddon(name:String, addon: String) : Bool
        access(all) fun getNames() : [String]
        access(all) fun containsName(_ name: String) : Bool
    ...
    */
    access(all) resource Network {
        ...
        access(all) fun internal_renew(name: String) {
            if let lease= self.profiles[name] {
        ...
        access(all) fun getLeaseExpireTime(_ name: String) : UFix64{
            if let lease= self.profiles[name] {
        ...
        access(all) fun getLeaseLockedUntil(_ name: String) : UFix64{
            if let lease= self.profiles[name] {
        ...
        access(all) fun internal_register(name: String, profile: Capability<&{Profile.Public}>,  leases: Capability<&{LeaseCollectionPublic}>) {
        ...
        access(all) fun profile(_ name: String) : &{Profile.Public}? {
            let nameStatus=self.readStatus(name)
    ...
    access(all) fun getFlowUSDOracleAddress() : Address {
        // If only find can sign the trxns and call this function, then we do not have to check the address passed in.
        // Otherwise, would it be wiser if we hard code the address here?
        if FIND.account.address == 0x097bafa4e0b48eef {
            // This is for mainnet
    ...
    access(all) fun getMerchantAddress() : Address {
        // If only find can sign the trxns and call this function, then we do not have to check the address passed in.
        // Otherwise, would it be wiser if we hard code the address here?
