...
access(all) contract FlovatarDustCollectibleAccessory: NonFungibleToken {
    ...
    access(all) resource interface Public {
        ...
        access(all) fun getTemplate(): FlovatarDustCollectibleTemplate.CollectibleTemplateData
        access(all) fun getSvg(): String
        access(all) fun getSeries(): UInt64
        access(all) fun getRarity(): String
        access(all) fun getMetadata(): {String: String}
        access(all) fun getLayer(): UInt32
        access(all) fun getBasePrice(): UFix64
    ...
    access(all) resource NFT: NonFungibleToken.INFT, Public, ViewResolver.Resolver {
        ...
        return <-FlovatarDustCollectibleAccessory.createEmptyCollection(nftType: Type<@Collection>())
    })
            }
...
access(all) fun createEmptyCollection(): @{NonFungibleToken.Collection}{
 return <-create Collection() 
    ...
    access(all) resource interface CollectionPublic {
        ...
        access(all) fun borrowCollectibleAccessory(id: UInt64): &FlovatarDustCollectibleAccessory.NFT? {
...
access(all) fun createEmptyCollection(): @{NonFungibleToken.Collection}{
 return <-create Collection() 
    ...
    access(all) struct CollectibleAccessoryData {
        access(all) let id: UInt64
        access(all) let templateId: UInt64
        access(all) let mint: UInt64
        access(all) let name: String
    ...
    access(all) fun getSvgForComponent(address: Address, componentId: UInt64) : String? {
        let account = getAccount(address)
        if let componentCollection = account.capabilities.get(self.CollectionPublicPath)!.borrow<&FlovatarDustCollectibleAccessory.Collection>()  {
    ...
    access(all) fun getAccessory(address: Address, componentId: UInt64) : CollectibleAccessoryData? {
        let account = getAccount(address)
        if let componentCollection = account.capabilities.get(self.CollectionPublicPath)!.borrow<&FlovatarDustCollectibleAccessory.Collection>()  {
    ...
    access(all) fun getAccessories(address: Address) : [CollectibleAccessoryData] {
    ...
    access(all) fun createCollectibleAccessory(templateId: UInt64, vault: @{FungibleToken.Vault}) : @FlovatarDustCollectibleAccessory.NFT {
        pre {
	...
	init() {
        ...
        self.account.storage.save<@{NonFungibleToken.Collection}>(<- FlovatarDustCollectibleAccessory.createEmptyCollection(nftType: Type<@Collection>()), to: FlovatarDustCollectibleAccessory.CollectionStoragePath)
        var _capForLinked1 = self.account.capabilities.storage.issue<&{FlovatarDustCollectibleAccessory.CollectionPublic}>( FlovatarDustCollectibleAccessory.CollectionStoragePath)
self.account.capabilities.publish(_capForLinked1 , at:FlovatarDustCollectibleAccessory.CollectionPublicPath)
