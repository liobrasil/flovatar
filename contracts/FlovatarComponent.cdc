...
access(all) contract FlovatarComponent: NonFungibleToken {
    ...
    access(all) resource interface Public {
        ...
        access(all) fun getTemplate(): FlovatarComponentTemplate.ComponentTemplateData
        access(all) fun getSvg(): String
        access(all) fun getCategory(): String
        access(all) fun getSeries(): UInt32
        access(all) fun getRarity(): String
        access(all) fun isBooster(rarity: String): Bool
        access(all) fun checkCategorySeries(category: String, series: UInt32): Bool
    ...
    access(all) resource NFT: NonFungibleToken.INFT, Public, ViewResolver.Resolver {
        ...
        return <-FlovatarComponent.createEmptyCollection(nftType: Type<@Collection>())
    })
            }
...
access(all) fun createEmptyCollection(): @{NonFungibleToken.Collection}{
 return <-create Collection() 
    ...
    access(all) resource interface CollectionPublic {
        ...
        access(all) fun borrowComponent(id: UInt64): &FlovatarComponent.NFT? {
...
access(all) fun createEmptyCollection(): @{NonFungibleToken.Collection}{
 return <-create Collection() 
    ...
    access(all) fun getSvgForComponent(address: Address, componentId: UInt64) : String? {
        ...
        if let componentCollection = account.capabilities.get(self.CollectionPublicPath)!.borrow<&{FlovatarComponent.CollectionPublic}>()  {
    ...
    access(all) fun getComponent(address: Address, componentId: UInt64) : ComponentData? {
        let account = getAccount(address)
        if let componentCollection = account.capabilities.get(self.CollectionPublicPath)!.borrow<&{FlovatarComponent.CollectionPublic}>()  {
    ...
    access(all) fun getComponents(address: Address) : [ComponentData] {
	...
	init() {
        ...
        self.account.storage.save<@{NonFungibleToken.Collection}>(<- FlovatarComponent.createEmptyCollection(nftType: Type<@Collection>()), to: FlovatarComponent.CollectionStoragePath)
        var _capForLinked1 = self.account.capabilities.storage.issue<&{FlovatarComponent.CollectionPublic}>( FlovatarComponent.CollectionStoragePath)
self.account.capabilities.publish(_capForLinked1 , at:FlovatarComponent.CollectionPublicPath)
