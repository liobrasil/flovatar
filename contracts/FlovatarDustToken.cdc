...
access(all) contract FlovatarDustToken: FungibleToken {
...
access(all) view fun isSupportedVaultType(type: Type): Bool{
 panic("implement me") 
    ...
    access(all) resource Minter {
        ...
        ///
        access(all) fun mintTokens(amount: UFix64): @FlovatarDustToken.Vault {
            pre {
    ...
    init() {
        ...
        //
        var _capForLinked1 = self.account.capabilities.storage.issue<&{FungibleToken.Receiver, FungibleToken.Balance}>( self.VaultStoragePath)
self.account.capabilities.publish(_capForLinked1 , at:self.VaultReceiverPath)


        // Create a public capability to the stored Vault that only exposes
        // the `balance` field through the `Balance` interface
        //
        var _capForLinked2 = self.account.capabilities.storage.issue<&FlovatarDustToken.Vault>( self.VaultStoragePath)
self.account.capabilities.publish(_capForLinked2 , at:self.VaultBalancePath)
