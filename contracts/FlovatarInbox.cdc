...
import HybridCustody from "hc/HybridCustody"
...
access(all) contract FlovatarInbox {
    ...
    access(all) event FlovatarDepositComponent(flovatarId: UInt64, componentId: UInt64)
    access(all) event FlovatarDepositDust(id: UInt64, amount: UFix64)
    access(all) event FlovatarWithdrawComponent(flovatarId: UInt64, componentId: UInt64, to: Address)
    access(all) event FlovatarWithdrawDust(id: UInt64, amount: UFix64, to: Address)
    access(all) event WalletDepositComponent(address: Address, componentId: UInt64)
    access(all) event WalletDepositDust(address: Address, amount: UFix64)
    access(all) event WalletWithdrawComponent(address: Address, componentId: UInt64)
    access(all) event WalletWithdrawDust(address: Address, amount: UFix64)
    access(all) event FlovatarClaimedCommunityDust(id: UInt64, amount: UFix64, to: Address)

    // This struct contains all the information about the Dust the be claimed for each Flovatar by the user
    access(all) struct ClaimableDust{
        access(all) let amount: UFix64
        access(all) let days: UInt64
        access(all) let rarityScore: UFix64
    ...
    access(all) resource Container {
        ...
        access(all) fun insertComponent(component: @FlovatarComponent.NFT) {
    ...
    access(all) resource interface CollectionPublic {
        access(all) fun depositDustToFlovatar(id: UInt64, vault: @{FungibleToken.Vault})
        access(all) fun depositDustToWallet(address: Address, vault: @{FungibleToken.Vault})
        access(all) fun depositComponentToFlovatar(id: UInt64, component: @FlovatarComponent.NFT)
        access(all) fun depositComponentToWallet(address: Address, component: @FlovatarComponent.NFT)
        access(all) fun getFlovatarDustBalance(id: UInt64): UFix64
        access(all) fun getWalletDustBalance(address: Address): UFix64
        access(all) fun getFlovatarComponentIDs(id: UInt64): [UInt64]
        access(all) fun getWalletComponentIDs(address: Address): [UInt64]
        access(all) fun getLastClaimedDust(id: UInt64): UFix64
    }
    ...
    access(all) resource Collection: CollectionPublic {
        ...
        access(all) var flovatarContainers: @{UInt64: FlovatarInbox.Container}
        access(all) var walletContainers: @{Address: FlovatarInbox.Container}
        ...
        access(all) var lastClaimedDust: {UInt64: UFix64}
        ...
        access(all) fun borrowFlovatarContainer(id: UInt64): &FlovatarInbox.Container {
        ...
        access(all) fun borrowWalletContainer(address: Address): &FlovatarInbox.Container {
        ...
        access(all) fun withdrawFlovatarComponent(id: UInt64, withdrawID: UInt64): @{NonFungibleToken.NFT} {
            let ref = self.borrowFlovatarContainer(id: id)
            return <- ref.withdrawComponent(id: withdrawID)
        }
        access(all) fun withdrawWalletComponent(address: Address, withdrawID: UInt64): @{NonFungibleToken.NFT} {
            let ref = self.borrowWalletContainer(address: address)
            return <- ref.withdrawComponent(id: withdrawID)
        }

        access(all) fun withdrawFlovatarDust(id: UInt64): @{FungibleToken.Vault} {
            let ref = self.borrowFlovatarContainer(id: id)
            return <- ref.dustVault.withdraw(amount: ref.dustVault.balance)
        }
        access(all) fun withdrawWalletDust(address: Address): @{FungibleToken.Vault} {
            let ref = self.borrowWalletContainer(address: address)
            return <- ref.dustVault.withdraw(amount: ref.dustVault.balance)
        }
    ...
    access(all) fun getFlovatarDustBalance(id: UInt64): UFix64 {
        if let inboxCollection = self.account.capabilities.get(self.CollectionPublicPath)!.borrow<&{FlovatarInbox.CollectionPublic}>()  {
    ...
    access(all) fun withdrawFlovatarComponent(id: UInt64, address: Address) {
        pre {
        	self.withdrawEnabled : "Withdrawal is not enabled!"
        }
        if let inboxCollection = self.account.storage.borrow<&FlovatarInbox.Collection>(from: self.CollectionStoragePath) {
    ...
    access(all) fun withdrawWalletComponent(address: Address) {
        pre {
        	self.withdrawEnabled : "Withdrawal is not enabled!"
        }
        if let inboxCollection = self.account.storage.borrow<&FlovatarInbox.Collection>(from: self.CollectionStoragePath) {
    ...
    access(all) fun withdrawFlovatarDust(id: UInt64, address: Address) {
        pre {
            self.withdrawEnabled : "Withdrawal is not enabled!"
        }
        if let inboxCollection = self.account.storage.borrow<&FlovatarInbox.Collection>(from: self.CollectionStoragePath) {
            if let flovatar = Flovatar.getFlovatar(address: address, flovatarId: id){
                let receiverAccount = getAccount(address)
                let receiverRef = receiverAccount.capabilities.get(FlovatarDustToken.VaultReceiverPath)!!.borrow<&{FungibleToken.Receiver}>()
    ...
    access(all) fun withdrawWalletDust(address: Address) {
        pre {
            self.withdrawEnabled : "Withdrawal is not enabled!"
        }
        if let inboxCollection = self.account.storage.borrow<&FlovatarInbox.Collection>(from: self.CollectionStoragePath) {
    ...
    access(all) fun getCommunityDustBalance(): UFix64 {
    ...
    access(all) fun getClaimableFlovatarCommunityDust(id: UInt64, address: Address): ClaimableDust? {
    ...
    access(all) fun claimFlovatarCommunityDust(id: UInt64, address: Address) {
        pre {
            self.withdrawEnabled : "Withdrawal is not enabled!"
        }
    ...
    access(all) fun claimFlovatarCommunityDustFromChild(id: UInt64, parent: Address, child: Address) {
        pre {
            self.withdrawEnabled : "Withdrawal is not enabled!"
        }
        ...
        let manager = getAccount(parent).capabilities.get<&HybridCustody.Manager>(HybridCustody.ManagerPublicPath)!.borrow()
    ...
    access(all) fun depositCommunityDust(vault: @{FungibleToken.Vault}) {
        pre {
            vault.isInstance(Type<@FlovatarDustToken.Vault>()) : "Vault not of the right Token Type"
        }
    ...
    access(all) fun getDustPerDayPerScore(): UFix64 {
    ...
    access(all) resource Admin {
        ...
        access(all) fun setDustPerDayPerScore(value: UFix64) {
            FlovatarInbox.setDustPerDayPerScore(value: value)
        }
        access(all) fun setWithdrawEnable(enabled: Bool) {
	...
	init() {
        ...
        self.account.storage.save<@FlovatarInbox.Collection>(<- FlovatarInbox.createEmptyCollection(), to: FlovatarInbox.CollectionStoragePath)
        var _capForLinked1 = self.account.capabilities.storage.issue<&{FlovatarInbox.CollectionPublic}>( FlovatarInbox.CollectionStoragePath)
self.account.capabilities.publish(_capForLinked1 , at:FlovatarInbox.CollectionPublicPath)
